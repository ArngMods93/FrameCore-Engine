...
import backend.ColorAccessibility;
...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		// For note skins and splash skins previews
		...
		// Colorblind Filter Option
		var option:Option = new Option('Colorblind Mode',
			'Sets colorblind accessibility filter for the entire game.',
			'colorblindMode',
			STRING,
			ColorAccessibility.options);
		option.onChange = function() {
			ColorAccessibility.apply(ClientPrefs.data.colorblindMode);
		};
		addOption(option);
